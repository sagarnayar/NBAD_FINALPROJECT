...
function ConfigurePage({ updateHomePageData }) {
  ...
  useEffect(() => {
    const fetchAllCategories = async () => {
      try {
        const response = await axios.get('http://64.176.213.182:3002/api/get-all-categories');
  ...
  const handleAllocationSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://64.176.213.182:3002/api/configure-budget', budgetData);
