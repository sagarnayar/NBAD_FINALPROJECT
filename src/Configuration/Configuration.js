...
function ConfigurePage({ updateHomePageData }) {
  ...
  useEffect(() => {
    const fetchAllCategories = async () => {
      try {
        const response = await axios.get('http://155.138.223.29:3002/api/get-all-categories');
  ...
  const handleAllocationSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://155.138.223.29:3002/api/configure-budget', budgetData);
