...
function HomePage() {
  ...
  const fetchData = (year, month, userId) => {
    axios
      .get(`http://45.77.107.121:3002/api/get-budgets/${year}/${month}/${userId}`)
      .then((response) => {
  ...
  const fetchTableData = () => {
    ...
    axios
      .get(
        `http://45.77.107.121:3002/api/get-table-data/${selectedYearTable}/${storedUserId}`
      )
      .then((response) => {
  ...
  const fetchUsedCategories = () => {
    axios
      .get("http://45.77.107.121:3002/api/get-all-categories")
      .then((response) => {
...
// eslint-disable-next-line
  const handleUsedBudgetSubmit = async (e) => {
    ...
    try {
      await axios.post("http://45.77.107.121:3002/api/enter-used-budget", payload);
